package uz.gita.dima.qosiqlar.presenter.base
...
import uz.gita.dima.qosiqlar.player.PlayerService
...
abstract class BaseSongActivity<P : BasePresenter<*>> : BaseActivity<P>(), PropertyChangeListener {
    ...
    override fun onStart() {
        ...
        if ((hasPermission(READ_MEDIA_AUDIO) || hasPermission(READ_EXTERNAL_STORAGE) || hasPermission(POST_NOTIFICATIONS)) && !isBound) {
    ...
    private fun checkPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            if (!hasPermission(READ_MEDIA_AUDIO)) {
                requestPermission(REQUEST_READ_MEDIA_AUDIO, READ_MEDIA_AUDIO, POST_NOTIFICATIONS)
            }
        } else if (!hasPermission(READ_EXTERNAL_STORAGE)) {
            requestPermission(REQUEST_WRITE_EXTERNAL_STORAGE, READ_EXTERNAL_STORAGE, POST_NOTIFICATIONS)
        } else if (!hasPermission(POST_NOTIFICATIONS)) {
            requestPermission(REQUEST_WRITE_EXTERNAL_STORAGE, READ_EXTERNAL_STORAGE, POST_NOTIFICATIONS)
        }
    }
